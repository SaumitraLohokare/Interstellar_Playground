...
public class Deflector {
	...
	private final double animDelay = 0.1d;
	private int animIndex = 1;

	public Deflector(int x, int y, int mass, int zoneRadius, Player target) {
		this.mass = mass;
	...
	private void updateAnim(double dt) {
		time += dt;
	...
	public void updateEditor(double dt) {
		...
		if (selected || placed) {
			if (Math.sqrt(Vec2.distSqr(pos, target.getPos())) <= zoneRadius) {
				target.setInField(true);

				Vec2 temp = pos.add(target.getPos().scalarMultiply(-1f));// removed scalar multiply -1
				target.addAcc(temp
						.scalarMultiply((float) ((mass * target.mass)
	...
	public void update(double dt) {
		...
		if (placed) {
			if (Vec2.distSqr(pos, target.getPos()) <= zoneRadius * zoneRadius) {
				target.setInField(true);				
				Vec2 temp = pos.add(target.getPos().scalarMultiply(-1f)); // removed scalar multiply
				target.addAcc(temp
						.scalarMultiply((float) ((mass * target.mass)
								/ (Vec2.distSqr(pos, target.getPos()) * Math.sqrt(temp.magnitudeSqr()))))
						.scalarMultiply(-1f));
	...
	public void renderEditor() {
		...
		if (!selected) {
			...
			Screen.drawImage(getAnimFrame(), (int) pos.x - (width / 2), (int) pos.y - (height / 2), width, height);
		} else {
			Screen.drawImage(getAnimFrame(), (int) pos.x - (width / 2), (int) pos.y - (height / 2), width, height,
